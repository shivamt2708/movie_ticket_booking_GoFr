...
func MyBookings2(ctx *gofr.Context) (interface{}, error) {
		email := ctx.PathParam("email")
		var customers []models.Booking

		// Getting the customer data from the database using SQL
		rows, err := ctx.DB().QueryContext(ctx.Request().Context(), "SELECT * FROM bookings WHERE user_email = ? ORDER BY id",email)
		if err != nil {
			return nil, err
		}
		defer rows.Close()
